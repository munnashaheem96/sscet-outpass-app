...
class ClassAdvisorApprovalScreen extends StatelessWidget {
  ...
  Future<Map<String, dynamic>> _fetchAdvisorDetails() async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        future: _fetchAdvisorDetails(),
        builder: (context, advisorSnapshot) {
          if (!advisorSnapshot.hasData) {
            return Center(child: CircularProgressIndicator());
          }

          final advisorData = advisorSnapshot.data!;
          final advisorYear = advisorData['year'];
          final advisorDepartment = advisorData['department'];

          return StreamBuilder(
            stream: FirebaseFirestore.instance
                .collection('outpass_requests')
                .where('class_advisor_status', isEqualTo: 'Pending')
                .where('year', isEqualTo: advisorYear)
                .where('department', isEqualTo: advisorDepartment)
                .snapshots(),
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => CaRequestDetailsScreen(requestId: docId)),
                            );
                          },
